...
function getContentFormatStats(videos) {
  const durations = videos.map((video) => parseIsoDurationToSeconds(video.duration));
...
function estimateRpm(channel, videos) {
  ...
  const isMostlyShorts =
    formatStats.contentFormat === "Shorts" || formatStats.contentFormat === "Mixed";
...
function calculateBasicMetrics(channel, videos) {
  ...
  const newestAverage =
    newest10.length > 0
      ? newest10.reduce((sum, video) => sum + video.views, 0) / newest10.length
      : 0;

  const olderAverage =
    older20.length > 0
      ? older20.reduce((sum, video) => sum + video.views, 0) / older20.length
      : 0;
  ...
  try {
    if (!YOUTUBE_API_KEY) {
      return res.status(500).json({
        error: "YOUTUBE_API_KEY не знайдено у файлі .env"
      });
    }
